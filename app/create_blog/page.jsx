...
const CreateBlog = () => {
  ...
  const {values, touched, errors, handleChange, handleSubmit} = useFormik({
    ...
    onSubmit :async (value , action)=>{
      value.tags = addedtag
      try {
        const res = await api.post('api/blog/create_blog', value)
        if (res.data.success) {
  ...
  <form className="mt-8 space-y-6 border pb-6 px-3 pt-2" action="" onSubmit={handleSubmit}>
    ...
    <div className=' flex flex-wrap justify-center gap-2'>
      {arr.map((tag)=> (
        <p key={tag} onClick={()=>handleAdd(tag)} className={`border text-center w-[120px] px-3 py-1 max-sm:text-sm max-sm:w-[60px] max-sm:px-1 cursor-pointer ${addedtag.includes(tag) ? " bg-red-400" : ""}`}>{tag} {addedtag.includes(tag) ? <IoRemoveCircleOutline className=' inline' /> : ''}</p>
      ))}
    </div>
    <div>
    ...
    <div className=' flex justify-center'>
      <input type="submit" value={'Upload Blog'} className="w-[200px] p-3 bg-[#fdc300] text-gray-800 text-lg rounded-md hover:bg-[#ffde72] cursor-pointer"/>
    </div>
  </form>
</div>
